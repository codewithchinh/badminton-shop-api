...
function toSkuPart(value: string) {
  return value
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .replace(/đ/g, 'd')
...
export function ProductManager({ brands, categories }: ProductManagerProps) {
  ...
  const selectedBrand = brands.find((brand) => brand.id === Number(productBrandId))
  ...
  function buildSuggestedSku() {
    const brandPart = toSkuPart(selectedBrand?.name ?? '') || 'THUONGHIEU'
    const productPart = toSkuPart(productName) || 'TENSANPHAM'
    let specPart = 'BIENTHE'

    if (isRacketCategory) {
      specPart = `${toSkuPart(variantWeight) || 'U'}${toSkuPart(variantGripSize) || 'G'}`
    }

    if (isShoeCategory) {
      specPart = toSkuPart(variantShoeSize) || 'SIZE'
    }

    return `${brandPart}-${productPart}-${specPart}`
  }
  ...
  useEffect(() => {
    if (!isSkuManuallyEdited) {
      setVariantSku(buildSuggestedSku())
    }
  }, [
    productName,
    productBrandId,
        ...
        <label>
          ...
          <input
            ...
            onChange={(event) => {
              ...
              setIsSkuManuallyEdited(event.target.value.trim() !== '')
            }}
            placeholder="YONEX-ASTROX88DPRO-4UG5"
            required
          />
        </label>
